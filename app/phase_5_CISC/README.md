...
## 8. Getting Started (Installation & Setup Guide)

Welcome to the team! Follow these steps to get the NFLIP platform running locally on your machine from scratch.

### Pre-requisites
- **Git** installed on your system.
- **Python 3.12+** (For the FastAPI & ML backend).
- **Node.js 18+** & npm (For the Next.js 14 frontend).

### Step 1: Clone the Repository
Open your terminal and pull down the codebase:
```bash
git clone <your-repository-url>
cd CISC/operation-room
```

### Step 2: Backend Setup (FastAPI + DuckDB)
The backend manages data ingestion, machine learning, and DuckDB vaults.
...
2. Create and activate a Python virtual environment:
   ...
   - **Mac/Linux:**
     ...
     ```
3. Install the required Python dependencies (including `cryptography`, `rfc3161ng`, and ML libraries):
   ```bash
...
4. Start the backend development server:
   ...
   ```
   *The backend API is now running at `http://localhost:8000`. You can view the swagger documentation at `http://localhost:8000/docs`.*

### Step 3: Frontend Setup (Next.js 14 Studio V4)
The frontend powers the Canva-style Studio canvas and UI workflows.

1. Open a **new terminal window/tab** and navigate to the frontend directory:
   ```bash
   cd CISC/operation-room/frontend
   ```
...
3. Start the frontend development server:
   ...
   npm run dev
   ```
   *The Studio UI will now be available at `http://localhost:3000`.*

### Step 4: External Services (Optional but Recommended)
- **Local LLM (Ollama):** If you plan to use local AI assistance for the ML correlation analysis and autonomous report ghost-writing, download and install [Ollama](https://ollama.com/), run it locally, and pull the required model (e.g. `ollama run llama3`).
- **Network Permissions:** The backend report exporter relies on the Time-Stamp Authority `http://freetsa.org/tsr`. Ensure your outbound firewall allows HTTP traffic to this domain for cryptographic `.zip` sealing to pass. 
>>>>>>> 42b34fe (set-up read.me)
